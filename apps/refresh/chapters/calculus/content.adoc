...
For h(x) = f(x) + g(x), the derivative is h'(x) = f'(x) + g'(x). Positive and negative rates can reinforce or cancel each other. The signed contributions share one scale, so their lengths can be compared directly.
...
For h(x) = f(g(x)), first let u = g(x). The inner slope is du/dx = g'(x), and the outer slope is dh/du = f'(u), evaluated at u = g(x), not at x. Their product gives dh/dx. Swapping f and g generally changes both the value and the derivative. Each plot's default range is centered on its evaluation input and uses its own fitted axes; slope values, rather than visual angles across plots, determine the chain-rule factors.
